...
export default function UseCases() {
  ...
  return (
    <section id="use-cases" style={{ background: 'var(--color-surface-base)', borderTop: '1px solid var(--color-cream-faint)', borderBottom: '1px solid var(--color-cream-faint)' }}>
      <div style={{ 
        ...
        <div style={{ 
          ...
          {cases.map((c, i) => {
            ...
            return (
              <div key={i} style={{
                padding,
                borderBottom: isLastRow ? 'none' : '1px solid var(--color-cream-faint)',
                borderLeft: isMiddleCol ? '1px solid var(--color-cream-faint)' : 'none',
                borderRight: isMiddleCol ? '1px solid var(--color-cream-faint)' : 'none',
                transition: 'background 0.15s',
              }}
                onMouseEnter={e => e.currentTarget.style.background = 'var(--color-cream-soft)'}
                onMouseLeave={e => e.currentTarget.style.background = 'transparent'}
              >
                <div style={{ fontSize: 11, color: 'var(--color-text-faint)', letterSpacing: '2px', marginBottom: 20 }}>{c.n}</div>
                <div style={{ fontSize: 17, color: 'var(--color-text-bright)', marginBottom: 10, fontFamily: 'Instrument Serif, serif', fontStyle: 'italic', lineHeight: 1.3 }}>{c.t}</div>
                <div style={{ fontSize: 13, color: 'var(--color-text-dim)', lineHeight: 1.7 }}>{c.d}</div>
                <span style={{ display: 'inline-block', fontSize: 10, letterSpacing: '1px', textTransform: 'uppercase', color: 'var(--color-text-dim)', border: '1px solid var(--color-cream-faint)', borderRadius: 4, padding: '3px 8px', marginTop: 16 }}>{c.tag}</span>
              </div>
            )
          })}
        </div>
